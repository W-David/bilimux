<template>
  <div class="mx-auto w-full flex flex-col gap-4">
    <div class="mt-12 flex flex-col px-8">
      <div
        class="mb-6 border-l-4 border-l-[#FB7299] border-l-style-solid pl-2 font-size-5 font-bold"
      >
        常规设置
      </div>
      <div class="flex flex-col gap-4 pl-4">
        ...
        <div class="flex items-center justify-between">
          ...
          <SelectButton v-model="preference['log-level']" :options="logLevelOptions" size="mini" />
        </div>
      </div>
    </div>
        ...
        <div class="flex flex-col gap-2">
          ...
          <InputGroup>
            <InputText
              ...
              placeholder="选择缓存目录"
            />
            <Button icon="i-mdi-folder-open text-lg" @click="selectCachePath" />
          </InputGroup>
        </div>
        ...
        <div class="flex flex-col gap-2">
          ...
          <InputGroup>
            <InputText
              ...
              placeholder="选择输出目录"
            />
            <Button icon="i-mdi-folder-open text-lg" @click="selectOutputDir" />
          </InputGroup>
        </div>
        ...
        <div class="flex flex-col gap-2">
          ...
          <InputGroup>
            <InputText
              ...
              placeholder="系统默认路径"
            />
            <Button icon="i-mdi-folder-open text-lg" @click="openGpacPath" />
          </InputGroup>
        </div>

        <div class="flex items-center justify-between">
          <label class="font-normal">强制转换 (即使文件已存在)</label>
          <ToggleSwitch v-model="preference['convert-config'].forceTransform" />
        </div>

        <div class="flex items-center justify-between">
          <label class="font-normal">强制合成 (即使文件已存在)</label>
          <ToggleSwitch v-model="preference['convert-config'].forceComposition" />
        </div>
      </div>
    </div>

    <div class="h-[80px] w-full flex justify-end gap-2 flex-items-center px-6">
      <Button label="重置" severity="secondary" text @click="clear" />
      <Button label="保存" icon="i-mdi-content-save" @click="save" />
    </div>
  </div>
</template>
...
const selectCachePath = async (): Promise<void> => {
  ...
  if (path) preference.value['convert-config'].cachePath = path
}
...
const selectOutputDir = async (): Promise<void> => {
  ...
  if (path) preference.value['convert-config'].outputDir = path
}
